...
## 通过类名来找这个类属于哪个dex multidex烦 小内存烦
...
def getImports(content):
    '''
    解析所有L开头的字符串，代表这是类字符串，但是有bug，Less也是L开头但不是类
    不知道如何解
    '''
    importRex = r'(L[^(]*?;)'
...
def parseFields(content):
    ...
    输出：
        成员变量的字典
    '''
    #一个变量包含修饰符，变量名，类型，初始值（如果是静态类型的话可能有）
...
def transClassName(name):
    '''
    处理L开头的类型字符串以及java基本类型，返回程序员可读的类型字符串
    输入：
        代表类型的字符串
    输出：
        以.分割的类表示
...
### 解析方法体 遍历每一条指令 解析感兴趣的指令，可以拓展
def parseMethodBody(content):
    '''
    解析方法体 遍历每一条指令 解析感兴趣的指令
    输入：
        content：方法体的字符串内容
    输出：
        这个方法体字典
    ...
    for instr in instructions:
        ...
        ### 解析const/int指令 const/4 const/16 const/high16 const/high16 p2, 0x42000000
        elif 'const ' in instr: #int
            constIntList.append(instr.split(',')[-1].strip())
            if constIntList:
                print('debug: constint {}'.format(constIntList))
                input()
        elif 'const/4 ' in instr: #int4
            constIntList.append(instr.split(',')[-1].strip())
            if constIntList:
                print('debug: constint 4 {}'.format(constIntList))
                input()
        elif 'const/16 ' in instr: #int4
            constIntList.append(instr.split(',')[-1].strip())
            if constIntList:
                print('debug: constint 16 {}'.format(constIntList))
                input()
        elif 'const/high16 ' in instr: #int4
            constIntList.append(instr.split(',')[-1].strip())
            if constIntList:
                print('debug: constint high16 {}'.format(constIntList))
                input()
        elif 'const-wide' in instr: #int4
            constIntList.append(instr.split(',')[-1].strip())
            if constIntList:
                print('debug: constint high16 {}'.format(constIntList))
                input()
    ...
    return invokeLists, constStrList, list(set(constIntList))
                
### 入口函数，将所有smali提取出来，形成一个app包，包含所有类的字段
def getInheritDict(pathList, outDictPath):
    '''
    入口函数，将所有smali提取出来，形成一个app包，包含所有类的字段
    输入：
        pathList：所有smali文件的路径列表
        outDictPath：app包的存储路径，app包达到几百兆，频繁写效率极低！
    输出：
        无
    '''
    idx = 0
...
### 解析一个smali文件，返回这个类的所有字段信息
def getInheritFromSmali(sPath):
    '''
    核心函数，用于生成一个smali文件中所有类信息的字典
    输入：
        smali文件的路径
    输出：
        这个类的特征
...
def traverseMethod(packageDict, iclassName, imethodName, iparams):
    '''
    遍历app包 找到对应的方法字典，如果没有找到，那么寻找这个类的父类，一直向上找
    输入：
        packageDict：app包
        iclassName, imethodName, iparams：这个方法的签名
    输出：  
        找到的方法字典
    '''
...
### 对app包进行全方法caller的寻找与相应字段的生成
def GenCallers(packageDict,androidCallerDict):
    '''
    对app包生成caller字段，存入每个函数dict中，并且syscall的caller单独形成一个包，
    因为syscall的类在app包中无法找到，例如a函数调用了 file.open() 那file.open函数的caller是a函数
    输入：
        packageDict：原始的app包
    输出：
        packageDict：直接在这个app包中添加caller字段
        androidCallerDict：系统函数的caller
    '''
            ...
            for invokeDict in invokeList:
                ...
                if iclassName.startswith('java.') or iclassName.startswith('android.') \
                    or iclassName.startswith('androidx.') \
                        or iclassName.startswith('javax.') or 'dalvik' in iclassName:
...
def GenInherit(packageDict):
    '''
    生成每个类的childclass字段，并且添加到主app包中
    输入：
        packageDict:原始的app包
    '''
    for clazz in packageDict:
