...
if __name__ == "__main__":
    ...
    for stuId in stuIdList:
        ...
        questionList = [')'.join(i.split(')')[0:-1])+')' for i in anwserMapList]
        anwserList = [i.split(')')[-1].strip() for i in anwserMapList]
        hashList = []
        for i in anwserList:
            m = hashlib.md5()
        ...
        writeList(anwserList, anwserPath)
